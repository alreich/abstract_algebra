...
def generate_symmetric_group(order, name=None, description=None, base=1):
    """Generates a symmetric group with the given order.
    ...
    description : str
      A description of the group. Defaults to
      'Autogenerated symmetric group of order n',
      where n is the group's order.
    base : integer
    ...
    -------
    Group
      A symmetric group of the given order. The elements will
      be permutations with the given order and base.
    """
    ...
    if description:
        ...
        desc = f"Autogenerated symmetric group of order {order}"
    ident = tuple(range(base, order + base))
