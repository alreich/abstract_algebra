...
def isomorphisms(alg0, alg1):
    """Return a list of isomorphisms (dictionaries) between
    the two input algebras"""

    # Get isomorphism between two large subalgebras from each algebra
    sub0 = None; sub1 = None
    subs = select_subalgebras(alg0, alg1)
    if subs is not None:
        sub0, sub1 = subs
    subs_iso = sub0.isomorphic(sub1)  # This is done via brute force (for now)

    # Find all possible mappings of remainder elements not in the subalgebra isomorphism
    ...
    # Append subalgebra isomorphism to each remainder mapping and look for full isomorphisms
    isos = [subs_iso | rem for rem in rem_dicts]
    return [iso for iso in isos if alg0.is_isomorphic_mapping(alg1, iso)]
